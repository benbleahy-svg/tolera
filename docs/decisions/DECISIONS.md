...
**Revisit trigger:** a facility-phone field landing in M5.8 (wire `%%FACILITY_PHONE%%`); the M6 webhook dispatcher consuming `domain_event`; the durable email-send outbox (closes the send-before-commit window); Fechner feedback on per-recipient link behaviour or the fallback sender.
