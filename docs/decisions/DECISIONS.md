...
**Known limitations (spec-faithful; recorded for the post-pilot tax-compliance / GoBD hardening pass, not v1 defects):**
- **Reverse charge is keyed on the buyer's USt-IdNr country prefix, not a structured ship-to/delivery country + transport evidence.** For *goods*, real intra-Community treatment (§6a) depends on the goods actually moving to another member state; §13b enumerates specific cases. The authoritative `DACH-DELTA-LAYER §3` defines the trigger as "selling to a VAT-registered business in another EU member state … requires the buyer's valid USt-IdNr" (VAT-ID-based), and structured addresses / §14 e-invoice fields are explicitly **post-v1**. v1 follows the spec's VAT-ID rule (plus the supplier-VAT-ID guard added in review); the delivery-country/transport-evidence gate is deferred to the compliance pass. Flagged by CodeRabbit.
- **Checkout is not idempotent server-side** — a network retry could create a second Order (the frontend disables the submit button on click, mitigating double-click). A per-quote idempotency key / token consumption is deferred (would need to reconcile with the M5.7 facilitated path and multi-order-per-quote). Not tier-1 (each Order is individually correct).
- **Structured tax-jurisdiction/fulfillment persistence** beyond free-text `billing_address` + the persisted tax classification (`reverse_charge`, `vat_rate_pct`, both VAT-IDs, `tax_rate_lines`, `vies_valid`/`vies_checked_at`) is post-v1.
