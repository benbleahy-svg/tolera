...
**Regression tests:** `tests/test_rules_m37.py::TestRegexIsBounded` (timeout enforced, fails closed, pattern logged without print text) and `::TestRegexSemanticsSurviveTheEngineSwap`; `tests/test_rules_m36.py::TestCatastrophicPatternsAreRejectedAtImport` and `::TestLegitimatePatternsSurviveTheRejectHeuristic` (the false-positive guard, incl. the nine §5 worked rules).
