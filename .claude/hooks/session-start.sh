...
0. ONE BLOCK = ONE SESSION. Start each block in a fresh session via /block <id>; after /ship, the session ends. Never run a second block in a used context. If HANDOFF.md or HANDOFF.autogen.md exists at the repo root, read it first. Ending mid-block? Run /handoff.
1. /block <id> — sweep stale worktrees, load only the block's sources, branch off develop, then GRILL me before coding.
