            ...
            (
                await session.execute(
                    select(PartFile).where(
                        ...
                        # M6.5 — a vendor's own quote PDF is filed against the part so
                        # the estimator sees it in Quote Files, but it is a competitor's
                        # price sheet: never forwardable to another vendor.
                        PartFile.storage_key.notin_(
                            select(VendorRfqResponse.attachment_object_key).where(
                                VendorRfqResponse.attachment_object_key.is_not(None)
                            )
