...
≈ **82 blocks** (+ deferred post-pilot work: a self-serve onboarding wizard and the M7 analytics milestone). *(2026-07-07: +4 — M4.9b/M4.10b material-scoped quoting; M6.7b thyssenkrupp materials4me; M6.7c CNC part quick-quote.)* *(2026-07-17: +5 from the decision-review session — M3.12 rules hardening (send gate); M3.13 ClamAV scanning; M4.7b provisional Core-4 goldens; M4.14 op-def formula evaluation; M5.0 estimating shell.)* At one block per focused session, that is the order of the pilot effort the spec scopes at 3–6 months solo + Claude Code. **M7 (Analytics)** is a resolved-scope but sizeable build (`DECISIONS.md` E4-k) that the spec's M0–M6 table doesn't slot; it is **not on the pilot critical path** and shares no golden-thread segment — sequence it during or after the pilot at your discretion.
...
- **Demos & screenshots** → via [DEMOS-TRACEABILITY.md](DEMOS-TRACEABILITY.md): the demo is the **acceptance oracle** — its narrative ([#demos](../docs/spec/Bid-Factory-Build-Spec.html#demos) / [#acceptance](../docs/spec/Bid-Factory-Build-Spec.html#acceptance)) plus the `../docs/reference/kb/screenshots/DemoX/` frames are the **UI ground truth** (build to them), and the fixture is the machine-checkable form (verify against it). Never inline demo text (DRY); the **DACH delta overrides** any US/imperial behaviour a screenshot shows.
