...
- **Scope (out):** auth, any domain table, any UI beyond a mounted shell placeholder (→ M0.2/M0.3).
...
- **Scope (out):** the reusable seed framework (→ M0.4); org-switcher UI (→ M0.3); real domain entities (→ M1).
- **Depends on:** M0.1
...
### M0.3 — App shell + navigation + i18n + BRAND   `[M]`  ⟂
...
### M0.4 — Seed framework + two orgs   `[M]`  ⟂
...
**M0 done when:** the cross-org denial + role-gated smoke tests pass (M0.2), the seed yields two orgs and login works (M0.4), the shell renders branded + localized (M0.3), and CI is green (M0.1). That is the spec's M0 exit criteria, met.
