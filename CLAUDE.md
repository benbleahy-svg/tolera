...
**The loop is autonomous end-to-end.** A human is pulled in only by (a) a **halt**: an `OPEN:` decision, a missing credential, an unmerged dependency, or CI still red after the driver's fix rounds — signalled via `BLOCKED.md` at the repo root; or (b) a **milestone checkpoint**: at each milestone boundary the driver runs `scripts/checkpoint.sh`, which boots the app with seeded data and hands the human a plain-language `CHECKPOINT.md` click-script — the build stays parked until `scripts/autobuild.sh --continue`.
...
6. **Machine + checkpoint gate (replaces per-PR human review):** the promoted Playwright demos in `e2e/` are the "human clicks the demo" gate in executable form and block every merge; cross-model review (`claude-code-review.yml`) puts the frontier model on any diff touching money/pricing/schema/auth/tax; and the human clicks through every milestone at the checkpoint before the next one starts.
