...
- **Demos are the acceptance oracle.** Via `DEMOS-TRACEABILITY.md`, a block's demo points to the narrative (`Bid-Factory-Build-Spec.html#demos` / `#acceptance`) and the `docs/reference/kb/screenshots/Demo<X>/` frames. The **screenshots are the UI ground truth** (layout, controls, copy) — build to them; the **fixture is the machine-checkable form** — verify against it.
...
## 9. Operating procedure — the autonomous per-block loop (hook-enforced)

Every build block runs this loop **in a fresh session — one block = one session, no exceptions**. Sessions are normally launched by the **driver, `scripts/autobuild.sh`**, which walks the build-plan queue block by block: one fresh headless session per block (perfect context hygiene by construction), Opus 4.8 for routine blocks and Fable 5 for risky ones (SPIKE/`[L]`, all of M4, anything touching Kalk/pricing/money/tax/schema/auth/tenancy — overridable in `build-plan/model-overrides.txt`), sleeping through usage-limit windows automatically. All durable state lives in git + `DECISIONS.md` + the PR (+ `HANDOFF.md` when a session dies mid-block), so a fresh session loses nothing. Local hooks in `.claude/hooks/` fire on their own; the test gate is bypassable with `CLAUDE_SKIP_TEST_GATE=1`.

**The loop is autonomous end-to-end.** A human is pulled in only by (a) a **halt**: an `OPEN:` decision, a missing credential, an unmerged dependency, or CI still red after the driver's fix rounds — signalled via `BLOCKED.md` at the repo root. **Milestone boundaries are non-blocking:** at each one the driver logs the finished milestone to `REVIEW-QUEUE.md` and continues straight into the next; the human reviews a completed milestone **on demand** by running `scripts/checkpoint.sh <M>`, which boots the app with seeded data and writes the plain-language `CHECKPOINT.md` click-script. The build never parks waiting for that review.

1. **`/block <id>`** — refuses a used session, loads only the block's sources (+ `HANDOFF.md` if resuming), branches off `develop`, then **self-grills** (`/grill-with-docs`): the grill questions are answered *from the sources* up the precedence ladder, every answer cited; uncited + cheap-to-reverse → `ASSUMED:`, uncited + expensive-to-reverse → `OPEN:` + halt. The verification table goes in the PR body. No human go-ahead — building starts immediately unless a halt fires.
...
5. **`/ship`** — runs ruff + mypy + pytest + **semgrep tier-1** (`.semgrep/tier1.yml`) + a **fresh-context subagent review** against `REVIEW.md` + CodeRabbit, opens the PR (with the verification table), **arms auto-merge** (`gh pr merge --auto --squash`), then ends the session. The PR merges itself when CI — backend · frontend · semgrep · e2e demos — is green. The **Stop hook** won't let a turn end with red lint/tests when code changed.
6. **Machine gate (replaces per-PR human review):** the promoted Playwright demos in `e2e/` are the "human clicks the demo" gate in executable form and block every merge; cross-model review (`claude-code-review.yml`) puts the frontier model on any diff touching money/pricing/schema/auth/tax. Milestone review is **non-blocking**: completed milestones queue up in `REVIEW-QUEUE.md` and the human clicks through one on demand via `scripts/checkpoint.sh <M>` — the next milestone starts without waiting.
